...
# additions and improvements Preston Holmes 2010
# further refactoring: John O'Neil December 2013
...
class Tweener:
  def __init__(self):
    ...
    self.currentTweens = []
    self.defaultTweenType = Tweener.IN_OUT_QUAD
  ...
  def OUT_IN_QUAD(t, b, c, d):
    if (t < d/2): 
      return Tweener.OUT_QUAD (t*2, b, c/2, d)
    return Tweener.IN_QUAD((t*2)-d, b+c/2, c/2, d)
  ...
  def OUT_IN_CUBIC(t, b, c, d):
    if (t < d/2): return Tweener.OUT_CUBIC(t*2, b, c/2, d)
    return Tweener.IN_CUBIC((t*2)-d, b+c/2, c/2, d)
